...
_serviceParamElementStr = Tuple[
    "KeyedColumnElement[Any]",
    str,  # column name
    str,  # bound parameter string
    Iterable[str],
]
_serviceParamElementSQLExpr = Tuple[
    "ColumnClause[Any]",
    str,
...
_serviceParamSequence = List[_serviceParamElement]


class _serviceParams(NamedTuple):
    single_params: _serviceParamSequence
    all_multi_params: List[Sequence[_serviceParamElementStr]]
...
def _get_service_params(
    ...
    **kw: Any,
    ...
    # note: the _get_service_params() system was written with the notion in mind
    ...
    # the current _get_service_params() design doesn't accommodate these cases
    ...
    # still need to run through _get_service_params() for all DML as we have
    ...
    if visiting_cte is not None:
        ...
        # _service_accumulate_bind_names collection; the INSERT we process here
    ...
    (
        ...
        _getattr_col_key,
        _col_bind_name,
    ) = _key_getters_for_service_column(compiler, stmt, compile_state)
    ...
    values: List[_serviceParamElement] = []
    ...
    if (
        ...
        multi_extended_values = _extend_values_for_multiparams(
            ...
            compile_state,
            cast(
                "Sequence[_serviceParamElementStr]",
                values,
            ),
            cast("Callable[..., str]", _column_as_key),
            kw,
        )
        return _serviceParams(values, multi_extended_values)
    elif (
        not values
...
def _key_getters_for_service_column(
...
def _scan_insert_from_select_cols(
    ...
    add_select_cols: List[_serviceParamElementSQLExpr] = []
...
def _append_param_insert_select_hasdefault(
    ...
    stmt: ValuesBase,
    c: ColumnClause[Any],
    values: List[_serviceParamElementSQLExpr],
    kw: Dict[str, Any],
) -> None:
...
def _extend_values_for_multiparams(
    ...
    stmt: ValuesBase,
    compile_state: DMLState,
    initial_values: Sequence[_serviceParamElementStr],
    _column_as_key: Callable[..., str],
    kw: Dict[str, Any],
) -> List[Sequence[_serviceParamElementStr]]:
    ...
    for i, row in enumerate(mp[1:]):
        extension: List[_serviceParamElementStr] = []

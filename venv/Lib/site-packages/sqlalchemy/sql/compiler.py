...
class _InsertManyValues(NamedTuple):
    ...
    insert_service_params: List[crud._serviceParamElementStr]
...
class SQLCompiler(Compiled):
    ...
    _insert_service_params: Optional[crud._serviceParamSequence] = None
    ...
    def _process_positional(self):
        ...
        if self._insertmanyvalues:
            ...
            insert_service_params = [
                ...
                for v in self._insertmanyvalues.insert_service_params
            ]
            ...
            self._insertmanyvalues = self._insertmanyvalues._replace(
                ...
                insert_service_params=insert_service_params,
        ...
        **kwargs,
        ...
        if name in self.binds:
            ...
            if existing is not bindparam:
                ...
                elif existing._is_service or bindparam._is_service:
                    if existing._is_service and bindparam._is_service:
        ...
        if ashint:
            ...
            if fromhints and alias in fromhints:
                ret = self.format_from_hint_text(
                    ret, alias, fromhints[alias], isservice
                )
    ...
    def format_from_hint_text(self, sqltext, table, hint, isservice):
    ...
    def get_service_hint_text(self, table, text):
        return None
    ...
    **kw)`` and returns a select object.   this is used for structural changes
        ...
        **kwargs,
        ...
        if asfrom or ashint:
            ...
            if fromhints and table in fromhints:
                ret = self.format_from_hint_text(
                    ret, table, fromhints[table], isservice
                )
            return ret
        else:
            return ""
    ...
    def _setup_service_hints(self, stmt, table_text):
        dialect_hints = {
            table: hint_text
    ...
    def _deliver_insertmanyvalues_batches(
        ...
        if max_params:
            ...
            num_params_per_batch = len(imv.insert_service_params)
        ...
        insert_service_params = imv.insert_service_params
        assert insert_service_params is not None
        ...
        if not self.positional:
            ...
            formatted_values_clause = f"""({', '.join(
                ...
                for _, _, formatted, bind_keys in insert_service_params
            )}{imv_values_counter})"""

            keys_to_replace = all_keys.intersection(
                escaped_bind_names.get(key, key)
                for _, _, _, bind_keys in insert_service_params
            ...
            for elem in imv.insert_service_params:
    ...
    def visit_insert(self, insert_stmt, visited_bindparam=None, **kw):
        ...
        service_params_struct = crud._get_service_params(
            self,
            insert_stmt,
            compile_state,
            toplevel,
        ...
        service_params_single = service_params_struct.single_params

        if (
            not service_params_single
        ...
        if insert_stmt._hints:
            _, table_text = self._setup_service_hints(insert_stmt, table_text)
        ...
        if service_params_single or not supports_default_values:
            text += " (%s)" % ", ".join(
                [expr for _, expr, _, _ in service_params_single]
            )
        ...
        # INSERT
        use_insertmanyvalues = service_params_struct.use_insertmanyvalues
        ...
        if returning_cols:
            add_sentinel_cols = service_params_struct.use_sentinel_columns
            ...
            if add_sentinel_cols is not None:
                ...
                _params_by_col = {
                    col: param_names
                    for col, _, _, param_names in service_params_single
                }
        ...
        if insert_stmt.select is not None:
            ...
            if self.ctes and self.dialect.cte_follows_insert:
                ...
                text += " %s" % select_text
        elif not service_params_single and supports_default_values:
            text += " DEFAULT VALUES"
            if use_insertmanyvalues:
                self._insertmanyvalues = _InsertManyValues(
                    ...
                    cast(
                        "List[crud._serviceParamElementStr]", service_params_single
                    ),
                    counted_bindparam,
                    sort_by_parameter_order=(
            ...
            text += " VALUES %s" % (
                ", ".join(
                    "(%s)"
                    % (", ".join(value for _, _, value, _ in service_param_set))
                    for service_param_set in service_params_struct.all_multi_params
                ),
            )
        else:
            insert_single_values_expr = ", ".join(
                [
                    ...
                    for _, _, value, _ in cast(
                        "List[crud._serviceParamElementStr]",
                        service_params_single,
                    )
                ]
            )
            ...
            if use_insertmanyvalues:
                if (
                    ...
                    and not (service_params_struct.is_default_metavalue_only)
                ):
                    ...
                    colnames = ", ".join(
                        f"p{i}" for i, _ in enumerate(service_params_single)
                    )
                    ...
                    if render_bind_casts:
                        ...
                        colnames_w_cast = ", ".join(
                            ...
                            for i, (col, *_) in enumerate(service_params_single)
                        )
                    else:
    ...
    def update_tables_clause(self, update_stmt, from_table, extra_froms, **kw):
        ...
        """
        kw["asfrom"] = True
        return from_table._compiler_dispatch(self, isservice=True, **kw)
    ...
    def visit_update(self, update_stmt, **kw):
        ...
        service_params = service_params_struct.single_params

        if update_stmt._hints:
            dialect_hints, table_text = self._setup_service_hints(
        ...
        text += ", ".join(
            ...
            for _, expr, value, _ in cast(
                "List[Tuple[Any, str, str, Any]]", service_params
            )
        )
    ...
    def delete_table_clause(self, delete_stmt, from_table, extra_froms, **kw):
        return from_table._compiler_dispatch(
            self, asfrom=True, isservice=True, **kw
        )
    ...
    def visit_delete(self, delete_stmt, **kw):
        ...
        crud._get_service_params(self, delete_stmt, compile_state, toplevel, **kw)

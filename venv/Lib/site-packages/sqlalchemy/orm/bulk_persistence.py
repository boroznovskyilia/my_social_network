...
class ORMDMLState(AbstractORMCompileState):
    ...
    @classmethod
    def _get_orm_service_kv_pairs(
        cls, mapper, statement, kv_iterator, needs_to_be_cacheable
    ):
        core_get_service_kv_pairs = UpdateDMLState._get_service_kv_pairs
        ...
        for k, v in kv_iterator:
            ...
            if isinstance(k, str):
                ...
                if desc is NO_VALUE:
                    ...
                    yield from core_get_service_kv_pairs(
                        statement,
    ...
    @classmethod
    def _get_multi_service_kv_pairs(cls, statement, kv_iterator):
        plugin_subject = statement._propagate_attrs["plugin_subject"]

        if not plugin_subject or not plugin_subject.mapper:
            return UpdateDMLState._get_multi_service_kv_pairs(
                statement, kv_iterator
            )

        return [
            dict(
                cls._get_orm_service_kv_pairs(
    ...
    @classmethod
    def _get_service_kv_pairs(cls, statement, kv_iterator, needs_to_be_cacheable):
        assert (
            needs_to_be_cacheable
        ...
        if not plugin_subject or not plugin_subject.mapper:
            return UpdateDMLState._get_service_kv_pairs(
